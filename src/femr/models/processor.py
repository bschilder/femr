...
class BatchCreator:
    ...
    def add_patient(self, 
        ...
        events_map = femr.pat_utils.get_event_indices(patient)

        for event_time, event_idx in events_map.items():
            
            # We want to avoid duplicate codes in the same day, so we maintain codes_seen_today
            if event_time is not None and event_time.date() != current_date:
                current_date = event_time.date()
                codes_seen_today = set()


            # Measurements are lists of dictionaries with a 'code' key
            # Each item in the list represents an event, during which there can be multiple measurements (multiple dictionaries with a 'code' key)
            event_measurements = femr.pat_utils.filter_dataset(patient, {"table_name": "measurement",
                                                                         "time": event_time})
            
            if len(event_measurements) == 0:
                raise ValueError(f"No measurements found for patient {patient['subject_id'][0]}")
            
            for measurement in event_measurements:

                # Get features and weights for the current event
                if event_time is None:
                    raise ValueError(f"No time found for patient {patient['subject_id'][0]}")
                
                features, weights = self.tokenizer.get_feature_codes(event_time, measurement)
                ...
                per_patient_ages.append((event_time - birth) / datetime.timedelta(days=1))
                per_patient_normalized_ages.append(self.tokenizer.normalize_age(event_time - birth))
                per_patient_timestamps.append(event_time.replace(tzinfo=datetime.timezone.utc).timestamp())
        ...
        self.patient_ids.extend([patient["subject_id"][0]] * length_to_add)
